...
static int a_eh_filho(TNO*r,void*chave,int(*cmp)(void*chave,void*dado)){
  TNO* filho = (TNO*)busca(r->filho,chave,cmp);
...
int agen_eh_filho(TAG* a,void* chave,void*chave_pai){
  if(a == NULL)return 0;
  TNO*pai = busca_no(a->raiz,chave,a->cmp);
  if(pai == NULL)return 0;
  return a_eh_filho(pai,chave_pai,a->cmp);
}
